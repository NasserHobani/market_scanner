...
#
# ═══ والفحص على المحاذاة لا على أوّل شمعة ═══
#
# الصيغة الأولى قارنت ``out.index[0]`` بأوّل شمعةٍ ساعية — وسقطت
# وهي مخطئة: ``limit=100`` يعيد آخر مئة شمعة، فأوّلُ المعروض ليس
# أوّل التاريخ. والكود كان سليماً.
#
# والمقصود أنّ الطوابع على حدود أربع ساعات من منتصف الليل — وهي
# علامة ``label="left"``. فالنسبة إلى النهاية تعطي ‎04:00‎ لشمعةٍ
# بدأت ‎00:00‎، وهي محاذاةٌ صحيحة أيضاً — فيُفحَص معها أنّ أوّل
# طابعٍ ≤ أوّل ساعيٍّ داخل **النافذة المعادة** لا التاريخ كلّه.
check("  والطوابع على حدود أربع ساعات",
      all(t.hour % 4 == 0 and t.minute == 0 for t in out.index),
      str([str(t) for t in out.index[:3]]))
_win_h = h[h.index >= out.index[0]]
check("  والشمعة تُنسَب إلى بدايتها",
      len(_win_h) and out.index[0] <= _win_h.index[0],
      f"{out.index[0]} مقابل {_win_h.index[0] if len(_win_h) else '—'}")
...
#
# سقط هذا الفحص لأنّ ``_Stub.fetch`` يعترض كل فريمٍ غير مشتقّ
# ويعيد الساعيّ — فلا يبلغ التحقّق في المحوّل الحقيقي أصلاً.
# اختبارٌ يلتفّ على ما يفحصه.
#
# فالنداء هنا على ``YahooAdapter`` نفسه: التحقّق يقع قبل أيّ
# شبكة، فلا حاجة إلى محاكاة.
try:
    Y.YahooAdapter().fetch("X", "3s")
    check("  والمجهول يرمي", False, "لم يرمِ")
except ValueError as exc:
    check("  والمجهول يرمي", "غير مدعوم" in str(exc), str(exc)[:80])
except Exception as exc:  # noqa: BLE001
    check("  والمجهول يرمي", False, f"{type(exc).__name__}: {str(exc)[:60]}")
