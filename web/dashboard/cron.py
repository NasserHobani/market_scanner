...
#
# ═══ ولماذا لم تعد الحارس الأوّل ═══
#
# وُضعت لأنّ ``next_run`` كان يتقدّم **بعد** العمل: تُشغَّل
# المهمّة، ثمّ تُكتب النتيجة. وفشلُ تلك الكتابة يترك المهمّة
# مستحقّةً وعملُها قد تمّ — فتُعاد، فتطيل احتكار القاعدة، فتفشل
# الكتابة ثانيةً. حلقةٌ تُغذّي نفسها، ظهرت بـ«القاعدة مقفلة» على
# أربع مهامّ دفعةً واحدة.
#
# وصار ``claim_due`` يقدّم الموعد **قبل** التشغيل — كما يفعل
# ``ir.cron`` في أودو. فالسبب زال من جذره.
#
# وتبقى هذه حزاماً ثانياً داخل العملية الواحدة، وحدُّها معروف:
# **لا تعبر الحاويات**. حاوية الويب وحاوية المجدول ذاكرتان
# منفصلتان — وهذا بالضبط ما يحرسه قفل الصفّ في القاعدة.
...
def _finish(job, status_: str, message: str, ms: int, *, manual: bool,
    ...
    # ═══ ولا تقديم هنا بعد اليوم ═══
    #
    # ``claim_due`` قدّمه قبل التشغيل. وتقديمُه ثانيةً هنا يقفز
    # فترةً كاملة زائدة: مهمّةُ الساعة تصير كل ساعتين، بلا خطأ
    # ولا أثرٍ في السجلّ — تعمل نصف ما ينبغي وتبدو سليمة.
    #
    # والتخطّي وحده يُعيد الضبط: هناك لم يجرِ عمل، فالتعجيل
    # مقصود.
...
def due_jobs():
    """المستحقّ للقراءة وحدها — العرض والتشخيص، لا التشغيل.

    والتشغيل يمرّ بـ``claim_due``: القراءةُ بلا حجزٍ تجعل عمليتين
    تريان المهمّة نفسها مستحقّةً فتشغّلانها معاً.
    """
    from django.utils import timezone
...
def claim_due(*, limit: int = 40) -> list:
    ...
    كان يقع بعده: تُشغَّل المهمّة، ثمّ يُكتب ``next_run``. وفشلُ
    تلك الكتابة — قفلُ قاعدةٍ أو انهيارُ عملية — يترك المهمّة
    مستحقّةً وعملُها قد تمّ. فتُعاد. ومسحٌ يستغرق اثنتي عشرة
    دقيقة يُعاد كاملاً، فيطيل احتكار القاعدة، فتفشل الكتابة
    ثانيةً: حلقةٌ تُغذّي نفسها — وقد وقعت.

    وعالجناها بذاكرةٍ **داخل العملية** (``_ran_at``). وهي لا تعبر
    الحدود: حاوية الويب وحاوية المجدول ذاكرتان منفصلتان، فكلٌّ
    تظنّ المهمّة لم تُشغَّل.

    و‏``ir.cron`` في أودو يقدّم ``nextcall`` داخل معاملة الحجز
    نفسها. فالانهيار بعدها يعني تفويت دورةٍ — لا إعادتها أبداً.
    وتفويتُ دورةٍ أهون بكثير.

    ═══ والحجز بقفل صفّ ═══

    ``FOR UPDATE SKIP LOCKED``: عمليةٌ تحجز الصفّ، والأخرى تتخطّاه
    فوراً بلا انتظار. وهو ما يمنع التشغيل المزدوج **عبر
    الحاويات** — لا ذاكرةٌ محلّية.

    و‏SQLite لا يدعمه. فيُكتفى هناك بالمعاملة وتقديم الموعد: كاتبٌ
    واحد أصلاً، فالسباق أضيق بكثير.

    ═══ والمعاملة قصيرة عمداً ═══

    أودو يُبقي القفل طوال التنفيذ. وهنا لا: مسحٌ يستغرق اثنتي
    عشرة دقيقة يُبقي معاملةً مفتوحة تلك المدّة، وهي وصفةُ
    الاحتكار الذي عانيناه. فالحجز يُقدّم الموعد ويُغلق، والتنفيذ
    خارجه — والموعد المتقدّم وحده يمنع غيرَنا.
    """
    ...
    now = timezone.now()
    skip_locked = getattr(connection.features,
    ...
    try:
        with transaction.atomic():
            ...
            for job in list(qs)[:limit]:
                # ‏``advance`` يقفز فتراتٍ كاملة حتى يتجاوز الآن:
                # مهمّةٌ توقّفت ساعةً تعمل **مرّة** لا ستّين.
                job.next_run = advance(job, now=now)
                job.last_run_at = now
                job.save(update_fields=["next_run", "last_run_at"])
                claimed.append(job)
    except Exception as exc:  # noqa: BLE001
        # قفلُ قاعدةٍ لحظة الحجز: لا شيء يُشغَّل هذه النبضة،
        # والمستحقّ يبقى مستحقّاً. وهو الاتّجاه الآمن — بخلاف
        # التشغيل بلا حجز.
        log.warning("تعذّر حجز المهامّ: %s", str(exc)[:140])
        return []
    return claimed
